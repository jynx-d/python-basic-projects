...
def save_high_score(name, score):
    ...
    scores = []
    try:
        with open("highscores.txt", "r") as f:  # Read all existing scores
            for line in f:
                og_name, og_score = line.strip().split(",")
                scores.append((og_name, int(og_score)))
    except FileNotFoundError:
        pass
    
    scores.append((name, score)) # Add new score
    
    scores.sort(key=lambda x: x[1], reverse=True) # Sort by score (descending)
    
    scores = scores[:5] # Keep only top 5 and overwrite file
    
    with open('highscores.txt', 'w') as f: # Write sorted top 5 back to file
        for n, s in scores:
            f.write(f"{n},{s}\n")
